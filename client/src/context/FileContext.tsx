...
function FileContextProvider({ children }: { children: ReactNode }) {
    ...
    const getSavedFileStructure = () => {
        try {
            const params = new URLSearchParams(window.location.search)
            const roomId = params.get("roomId") || window.location.pathname.split("/").pop()
            if (roomId) {
                const saved = localStorage.getItem(`fileStructure_${roomId}`)
                if (saved) return JSON.parse(saved)
            }
        } catch (err) {
            console.error("Error loading file structure from local storage", err)
        }
        return initialFileStructure
    }

    const [fileStructure, setFileStructure] = useState<FileSystemItem>(getSavedFileStructure())
    ...
    const [activeFile, setActiveFile] = useState<FileSystemItem | null>(openFiles[0] || null)

    useEffect(() => {
        const params = new URLSearchParams(window.location.search)
        const roomId = params.get("roomId") || window.location.pathname.split("/").pop()
        if (roomId && fileStructure) {
            localStorage.setItem(`fileStructure_${roomId}`, JSON.stringify(fileStructure))
        }
    }, [fileStructure])
