    ...
    const runCode = async () => {
        try {
            ...
            if (error?.response?.data) {
                ...
                toast.error(`Run failed: ${error.response.data.message || error.response.data.error || "Unknown API error"}`)
            } else {
                console.error("Network/CORS error running code:", error?.message || error)
                toast.error("Failed to run the code — check your internet connection or try a different language")
            }
            setIsRunning(false)
            toast.dismiss()
        }
